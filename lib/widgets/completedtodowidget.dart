...
class Completedtodowidget extends StatelessWidget {
  const Completedtodowidget({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    final provider =
        Provider.of<Todosprovider>(context); //per avere accesso a provider
    final alltodos = provider.todoscompleted;
    //per ottenere lista todo non fatti
    return alltodos.isEmpty
        ? Center(
            child: Text(
              'Nessun todo completato', //nel caso in cui non ho todo
              style: TextStyle(
